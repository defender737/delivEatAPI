...
public class OrderController {
    ...
    public ResponseEntity<String> addOrder(@PathVariable Long user_id){
        OrderDto order = orderService.addOrder(user_id);
        return ResponseEntity.status(HttpStatus.CREATED).body("주문이 완료되었습니다. 주문ID: " +  order.getOrderId());
    }
    ...
    public ResponseEntity <String> ChangeOrderStatus(@PathVariable Long order_id, @PathVariable String status) {
        OrderDto changedStatus = orderService.changeStatus(order_id, status);
        return ResponseEntity.status(HttpStatus.CREATED).body("주문이" + changedStatus.getStatus() + "되었습니다. 주문ID: " +  changedStatus.getOrderId());
    }
